...
> 本書の前半は書き換え前の初回監査記録である。追加照合後、利用者の承認に基づき
> 履歴クリーンアップを実施済み。現在の結果は[Phase 4a 照合走査](照合走査_Phase4a.md)を参照。
...
初回監査時点では、履歴の書き換え、force push、ブランチ／タグの削除、pushは実施していなかった。
...
|---|---|---:|---|
| `83a4b29029d226ac28574e0e3f67c3adb6eef70c` | `AISTATE.md`の導入差分 | 2 | 書き換え前の記録／現在は一般化済み |
| `4a0aef50e31c5ac8a5bf3fb2dcc532fb3c13f61a` | 到達不能blobの`AISTATE.md` | 2 | 書き換え前の記録／ローカルGC済み |
...
初回作業では、現在の追跡ファイルからuser固有pathを除去し、`.gitignore`回帰testと
現在treeのblocking走査をCIへ追加した。その後、追加指示で下記2の履歴書き換えが承認された。

過去履歴への選択肢は次のとおり。実施判断は利用者に委ねる。

1. **履歴を維持する**: cloneや既存tagを壊さない。過去のメール形式と絶対パス候補は
   公開履歴に残る。
