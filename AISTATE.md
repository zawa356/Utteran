...
> bugfix-dで単語時刻を破棄する処理を入れたが、33件が該当し続けており、根本原因
> （なぜ異常な時刻が生じるか）は未解決のままだった。

- v1.9.2/OpenVINO+Vulkanで1,484.8秒を再推論し、本文非出力診断で33件を再現した。28件は
  3秒超wordを含み、5件はword群の片端偏在だけだった。28件中25件には正常wordが残り、
  全wordが3秒超だったのは3件だけ。従来は1件の異常でもsegment内の全488 wordを捨てていた。
- 異常wordだけを除き、片端偏在はword包絡へ補正した。部分保持は本文位置を按分せずlow confidence
  の単一segmentとする。再測定はASR wordless 33→3、low confidence部分保持25件。残る3件・
  94.0秒は全wordが異常で救済不能。終了時刻の片端伸長5件はword包絡補正対象となった。
- upstreamは2026-08-28のHEAD `9781133`、最新binary releaseは`b4938`。masterにはVAD区間getterと
  token時刻の元timeline getterの回帰testがあるが、33件の大半を占める長時間word自体を直す
  release済み解決策は確認できなかった。未release commitへのpin変更は4構成再buildのriskが高く、
  v1.9.2 pinと既存patchを維持した。

### F-2〜F-5

> bugfix-eの形態素境界スナップにより、同一時刻を共有するトークン群を分割する事象が新たに
> 発生した（0.1.15で0件→0.1.16で1件）。この観点の回帰テストがなかったため検出できなかった。

- 同一時刻群の保護を形態素・文節スナップより優先する。回帰testを追加し、実測は複数話者群
  1→0、segment overlap 1→0。日本語gapは0.10秒未満だけを処理し、Sudachi品詞から副詞・
  連体詞後を文節候補として優先した。
- 末尾短応答は音響支持を必須として2候補を分離した。A/B/A再判定は逆方向の処理なので、両gap
  0.01秒未満に加えて外側話者の音響重なり優位を必須とした。実データでは0件で、既知3断片を
  根拠なく吸収しない安全側判断。F-3で支持された応答はF-5条件から除外される。

### F-6 調査のみ

- ASR segment自体が2話者を含む場合、word時刻とpyannote turnがあれば後段分割できるが、同一時刻
  token群またはwordless区間では話者交代位置を復元できない。ASRの句境界設定だけでは話者turnを
  保証せず、分離側で精度を捏造する解決はできない。指示どおり実装しなかった。

### 回帰測定

- 実データ最終結果は109 segment、wordless 3件・94.0秒、同一時刻群複数話者0、overlap 0、
  0.5秒未満0、segment合計1,290.33秒。本文・固有名詞・話者名は記録していない。意味評価を
...
> 話者境界を時刻だけで決めていたため、日本語では語や文節の内部に境界が落ちていた。ずれ幅が
> 例外なくASRトークン1個分であり、言語的な境界を考慮する必要があった。

- 実効ASR言語が`ja`で、単語gap 0.02秒以下の境界だけをSudachiPy／sudachidict_coreの文字位置へ
  最大4文字スナップする。助詞・助動詞・接尾辞・非自立動詞から始まる境界と接頭辞直後を候補から
  除き、同距離は観測された遅延biasに合わせ早い側を選ぶ。移動後wordの時刻からsegmentも更新する。
- `--language auto`はASRの確定結果が`ja`なら適用し、英語等は通さない。Sudachiがない／辞書初期化
  不能なら警告し、旧割当のまま続行する。GUI venvへ辞書を入れず、全推論profileへ`japanese` extraを
  追加した。既存profileはsetup再実行が必要。実増加は220,955,720 bytes（約210.7 MiB）。
- 1,484.8秒の実会議中間結果を本文非出力で再計算した。外部評価の22/77に加え付属語規則も含む
  機械判定では不適切境界26/77→3/73、segment 119→116。A/Bはいずれも32候補中27件・46文字を
  移動し同値だった。別の複合語試料ではAだけが細粒度候補を保持し、最大移動制約と相性がよいため
  指示どおりAを既定にした。移動時刻はmin 0.01、median 0.26、p95 1.55、max 2.76秒。

### E-2/E-3 単語時刻なしsegment

> bugfix-dで本文を保持する判断は正しかったが、segmentの時刻範囲をそのまま残したため、無音区間
> まで話者labelが伸びた。データを保持する際は、その信頼度も併せて扱う必要がある。

- whisper.cpp v1.9.2固定commitにはPR #3916相当の内部VAD speech segment公開APIがなく、公式full
  JSONにもVAD区間はない。2026-08-28時点のupstream masterにはgetterがあるが、native pin更新と
  追加patchを本修正へ混ぜず、既に取得済みのpyannote発話turnを原理的に同じ音響区間として使う。
- 単純な4文字/秒＋1秒上限は349.220秒を削れたが検出発話coverage 94.060%となり不採用。重なる
  pyannote turnの包絡を優先し、turnがない場合だけ同文字上限へfallbackする方式は、単語時刻なし
  33件・508.960秒→33件・356.079秒（152.881秒削減）、検出発話929.596/935.449秒、coverage
  99.374%。timeline占有率は無音除去により97.786%→88.743%となるが、本文segmentは1件も捨てない。
- 話者推定は実用性のため残し、JSON segmentへ`speaker_confidence=low`を付けるB/C方針とした。
  word時刻ありは`high`。SRT等は話者名を維持する。任意field追加で既存GUIが未知fieldを無視できる
  ためschema version 1を維持した。alignment policy versionは8。

### E-4 相槌分離（調査のみ）

- E-1は既存の話者交代境界を移すだけなので、同一label内に埋まった相槌を新規分離しない。ただし
  既にword speaker列へ現れた相槌の端が語中にある場合は副次的に改善する。
- whisper.cpp CLIは`--max-len`／`--split-on-word`の長さ分割を持つが疑問文終止形を指定できず、
  faster-whisperにも句読点callbackによる強制segment分割契約はない。ASR後に疑問符・終助詞で
  分割する実装は可能だが、17/19件がgap 0.02秒以下で音響話者根拠がなく、話者を復元できないまま
  過剰分割を増やす危険が高い。実装規模はASR共通後処理、時刻再配分、両backend・言語別回帰を含む
  中規模。本phaseでは指示どおり実装しない。

### E-5 話者cluster数（調査のみ）

- pyannote backendは`num_speakers`または`min_speakers`/`max_speakers`を公式pipelineへそのまま渡す。
  既存3分合成受入ではautoが内部4／最終3話者、固定2/3/4は各指定数、range 2〜5は4話者となり、
  制約がcluster結果へ反映されることを確認済み。3話者正解fixtureの最終3話者は妥当だった。
- 実会議の5 clusterと呼びかけ表記数の関係は、ASR表記揺れと同名／敬称を統計だけで除けず、音声
  聴取なしに実話者数を確定できない。本文解析結果は記録せず、実装・既定値変更は行わない。
